...
var serializerMiddleware = (options, serializer) => (next, context) => async (args) => {
  ...
  const endpoint2 = context.endpointV2?.url && endpointConfig.urlParser ? async () => endpointConfig.urlParser(context.endpointV2.url) : endpointConfig.endpoint;
  if (!endpoint2) {
    ...
    async pull(controller) {
      ...
      if (done) {
        ...
        if (checksumRequired) {
          ...
          controller.enqueue(`${checksumLocationName}:${checksum2}\r\n`);
          controller.enqueue(`\r\n`);
        }
        controller.close();
      } else {
...
class FetchHttpHandler {
  ...
  async handle(request, { abortSignal } = {}) {
    ...
    const url = `${request.protocol}//${auth3}${request.hostname}${port ? `:${port}` : ""}${path}`;
...
var resolveAwsSdkSigV4AConfig = (config2) => {
  config2.sigv4aSigningRegionSet = normalizeProvider3(config2.sigv4aSigningRegionSet);
  return config2;
};
...
var resolveAwsSdkSigV4Config = (config2) => {
  ...
  let signer;
  if (config2.signer) {
    signer = normalizeProvider3(config2.signer);
  } else if (config2.regionInfoProvider) {
    signer = () => normalizeProvider3(config2.region)().then(async (region) => [
    ...
    signer = async (authScheme) => {
      authScheme = Object.assign({}, {
        ...
        signingRegion: await normalizeProvider3(config2.region)(),
        properties: {}
      }, authScheme);
...
function normalizeCredentialProvider(config2, { credentials, credentialDefaultProvider }) {
  ...
  if (credentials) {
    ...
    if (credentialDefaultProvider) {
      credentialsProvider = normalizeProvider3(credentialDefaultProvider(Object.assign({}, config2, {
...
var flexibleChecksumsMiddleware = (config2, middlewareConfig) => (next, context) => async (args) => {
  ...
  if (checksumAlgorithm) {
    ...
    if (isStreaming(requestBody)) {
      const { getAwsChunkedEncodingStream: getAwsChunkedEncodingStream3, bodyLengthChecker } = config2;
      updatedBody = getAwsChunkedEncodingStream3(typeof config2.requestStreamBufferSize === "number" && config2.requestStreamBufferSize >= 8 * 1024 ? createBufferedReadable(requestBody, config2.requestStreamBufferSize, context.logger) : requestBody, {
        base64Encoder,
        bodyLengthChecker,
...
var validateChecksumFromResponse = async (response2, { config: config2, responseAlgorithms, logger: logger3 }) => {
...
var s3ExpressMiddleware = (options) => {
  return (next, context) => async (args) => {
    if (context.endpointV2) {
      const endpoint2 = context.endpointV2;
...
async function checkFeatures(context, config2, args) {
  ...
  const identity3 = context.__smithy_context?.selectedHttpAuthScheme?.identity;
  if (identity3?.$source) {
...
var resolveHttpAuthSchemeConfig = (config4) => {
  ...
  return Object.assign(config_1, {
    authSchemePreference: normalizeProvider(config4.authSchemePreference ?? [])
  });
};
...
var de_PutObjectCommand = async (output, context) => {
  ...
  const contents = map({
    ...
    [_SSEKMSEC]: [, output.headers[_xassec]],
    [_BKE]: [() => output.headers[_xassebke] !== undefined, () => parseBoolean(output.headers[_xassebke])],
    [_Si]: [() => output.headers[_xaos] !== undefined, () => strictParseLong(output.headers[_xaos])],
...
var package_default = {
  ...
  devDependencies: {
    "@aws-sdk/signature-v4-crt": "3.810.0",
...
var createDefaultUserAgentProvider = ({ serviceId, clientVersion }) => async (config4) => {
...
class EventStreamCodec {
  constructor(toUtf85, fromUtf88) {
    this.headerMarshaller = new HeaderMarshaller(toUtf85, fromUtf88);
    this.messageBuffer = [];
...
var getRuntimeConfig2 = (config4) => {
  ...
  return {
    ...
    credentialDefaultProvider: config4?.credentialDefaultProvider ?? ((_) => () => Promise.reject(new Error("Credential is missing"))),
...
var resolveHttpAuthRuntimeConfig = (config5) => {
  return {
...
class S3Client extends Client {
  ...
  constructor(...[configuration]) {
    ...
    this.middlewareStack.use(getHttpAuthSchemeEndpointRuleSetPlugin(this.config, {
      ...
      identityProviderConfigProvider: async (config5) => new DefaultIdentityProviderConfig({
...
var getSsecPlugin = (config5) => ({
  applyToStack: (clientStack) => {
    clientStack.add(ssecMiddleware(config5), ssecMiddlewareOptions);
  }
});
...
}).m(function(Command2, cs2, config5, o2) {
  return [
    ...
    getFlexibleChecksumsPlugin(config5, {
      requestAlgorithmMember: { httpHeader: "x-amz-sdk-checksum-algorithm", name: "ChecksumAlgorithm" },
      requestChecksumRequired: false
    }),
    getCheckContentLengthHeaderPlugin(config5),
    getThrow200ExceptionsPlugin(config5),
    getSsecPlugin(config5)
  ];
...
var BUCKET_NAME = "clickstream-analytics-bucket";
var PREFIX = "clickstream-events/";
async function handler(event) {
  ...
  for (const record of event.Records) {
    ...
    try {
      ...
      const response2 = await s3Client.send(command3);
      console.log(`Successfully saved to S3: ${response2}`);
    } catch (error) {
